	...
	<br>
		<table>
			...
			<tbody>
				...
				<c:forEach items="${list}" var="buyList">
					<tr>
						<td class="main_list_col1">이미지</td>
						<td class="main_list_col2">${buyList.name} 제목</td>
						<td class="main_list_col3">
							<!-- 각 제품의 할인된가격 총합 --> 
							<c:set var="discountPrice" value="${buyList.discountPrice} " /> 
							<strong><fmt:formatNumber value="${buyList.discountPrice}" pattern="#,###" /></strong>원 
							<span> | </span> 수량 ${buyList.cartStock} 개
							<div>${buyList.discountPrice * buyList.cartStock}</div>
							<div>
								[ ${buyList.discountRate} %↓ + ${buyList.bookPoint}원<span>P</span>]
							</div> 
