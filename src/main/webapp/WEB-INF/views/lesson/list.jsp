	...
	<div id="lessonList">
				...
				<div class="inner">
					...
					<ul>
						...
						<c:forEach items="${list}" var="lesson">
							...
							<li><input type="hidden" class="lessonId"
								data-order="${lessonCnt}" value="${lesson.id}">
								<div class="img">
									<div class="likeInfo">
										<a class="like" data-order="${lessonCnt}" href=""><i
											class="xi-heart-o"></i></a> <a class="reserv"
											data-order="${lessonCnt}" href=""><i class="xi-star-o"></i></a>
										<input type="hidden" class="isLike" value="false">
									</div>
									<a href="/lesson/read?id=${lesson.id }"> <c:if
											test="${empty lesson.thumbnail}">
											<img class="lessonImg"
												src="../../../resources/img/classtmpimg.jpg">
										</c:if> <c:if test="${!empty lesson.thumbnail}">
											<img class="lessonImg"
												src="/resources/img/lesson/thumb/${lesson.teacherId}${lesson.openAt}/${lesson.thumbnail}">
										</c:if>
								</div> <span>${lesson.typeName}</span><span>${lesson.categoryName}</span>
								<h3>${lesson.title}</h3>
								<div class="price">
									<fmt:formatNumber type="number" var="originPrice"
										maxFractionDigits="3" value="${lesson.originPrice}" />
									<h4>${originPrice}원</h4>
									<fmt:parseNumber var="discountRate" integerOnly="true"
										value="${lesson.discountRate }" />
									<h5>${discountRate}%</h5>
									<fmt:formatNumber type="number" var="salePrice"
										maxFractionDigits="3" value="${lesson.salePrice}" />
									<h1>${salePrice}원</h1>
								</div> </a></li>
						</c:forEach>
					</ul>
		...
		<form id='actionForm' action="/lesson/list" method='get'>
			...
			<input type='hidden' name='keyword'
				...
				type='hidden' name='categoryMain'
				value='<c:out value="${pageMaker.cri.categoryMain}"/>' /> <input
				type='hidden' name='categorySub'
				value='<c:out value="${pageMaker.cri.categorySub}"/>' />
		</form>
	</div>
	<script>
